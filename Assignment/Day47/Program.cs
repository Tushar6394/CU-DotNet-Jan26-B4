...
// Flatten the List<List<Container>> into a single List<Item>.
// Remove any duplicate items (based on Name).
// Sort the final list first by Category (alphabetical) and then by Weight (descending).
...
public class CargoManager
{
    ...
    public List<string> FindHeavyContainers(double weightThreshold)
    {
        ...
        return cargoBay.Where(row => row != null)
                       .SelectMany(row => row)
                       .Where(container => container.Items != null && container.Items.Sum(item => item.Weight) > weightThreshold)
