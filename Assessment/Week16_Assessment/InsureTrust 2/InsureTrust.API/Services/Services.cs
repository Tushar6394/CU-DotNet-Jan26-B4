...
public class AuthService : IAuthService
{
    ...
    public AuthService(IUserRepository repo, JwtHelper jwt, INotificationService notif)
    {
        _repo = repo; _jwt = jwt; _notif = notif;
    }
    ...
    {
        // Development-only: allow a hard-coded admin login for quick local testing.
        // This path is active only when ASPNETCORE_ENVIRONMENT == "Development".
    ...
    {
        Id = u.Id, UserNumber = u.UserNumber, Name = u.Name, Email = u.Email,
        PhoneNo = u.PhoneNo, Role = u.Role, KycStatus = u.KycStatus,
        Balance = u.Balance, CreatedAt = u.CreatedAt
    };

    private static bool IsValidPan(string pan)
    {
        if (string.IsNullOrWhiteSpace(pan) || pan.Length != 10) return false;
        var upper = pan.ToUpper();
        return upper[..5].All(char.IsLetter) &&
               upper[5..9].All(char.IsDigit) &&
               char.IsLetter(upper[9]);
    }
}
...
public class PolicyService : IPolicyService
{
    ...
    public PolicyService(IPolicyRepository repo, INotificationService notif, IUserRepository userRepo)
    {
        _repo = repo; _notif = notif; _userRepo = userRepo;
    }

    public async Task<IEnumerable<PolicyTypeDto>> GetPolicyTypesAsync()
    {
        var types = await _repo.GetAllPolicyTypesAsync();
        return types.Select(MapPolicyTypeDto);
    }
    ...
    public async Task<PolicyTypeDto> GetPolicyTypeByIdAsync(int id)
    {
        ...
        return MapPolicyTypeDto(pt);
    }

    public async Task<PolicyTypeDto> CreatePolicyTypeAsync(CreatePolicyTypeDto dto)
    {
        var pt = new PolicyType
        {
            Name = dto.Name, Category = dto.Category, Description = dto.Description,
            Icon = dto.Icon, BaseMonthlyPremium = dto.BaseMonthlyPremium,
            MinTenureMonths = dto.MinTenureMonths, MaxTenureMonths = dto.MaxTenureMonths,
            CoverageDetails = dto.CoverageDetails, IsActive = true, CreatedAt = DateTime.UtcNow
        };
        var created = await _repo.AddPolicyTypeAsync(pt);
        return MapPolicyTypeDto(created);
    }

    public async Task<PolicyTypeDto> UpdatePolicyTypeAsync(int id, CreatePolicyTypeDto dto)
    {
        var pt = await _repo.GetPolicyTypeByIdAsync(id) ?? throw new NotFoundException("Policy type not found.");
        pt.Name = dto.Name; pt.Category = dto.Category; pt.Description = dto.Description;
        pt.Icon = dto.Icon; pt.BaseMonthlyPremium = dto.BaseMonthlyPremium;
        pt.MinTenureMonths = dto.MinTenureMonths; pt.MaxTenureMonths = dto.MaxTenureMonths;
        pt.CoverageDetails = dto.CoverageDetails;
    ...
    {
        var policies = await _repo.GetUserPoliciesAsync(userId);
        return policies.Select(MapPolicyDto);
    }

    public async Task<IEnumerable<PolicyDto>> GetAllPoliciesAsync()
    {
        var policies = await _repo.GetAllPoliciesAsync();
        return policies.Select(MapPolicyDto);
    }

    public async Task<IEnumerable<PolicyDto>> GetPendingPoliciesAsync()
    {
        var policies = await _repo.GetPendingPoliciesAsync();
        return policies.Select(MapPolicyDto);
    }
    ...
    public async Task<PolicyDto> PurchaseAsync(CreatePolicyDto dto, int userId)
    {
        ...
        {
            UserId = userId, PolicyTypeId = dto.PolicyTypeId,
            Status = "Pending", PurchaseDate = DateTime.UtcNow,
            ExpiryDate = DateTime.UtcNow.AddMonths(dto.Tenure),
            Tenure = dto.Tenure, PackageAmount = dto.PackageAmount,
    ...
    private static PolicyTypeDto MapPolicyTypeDto(PolicyType pt) => new()
    {
        Id = pt.Id, Name = pt.Name, Category = pt.Category, Description = pt.Description,
        ...
        CoverageDetails = pt.CoverageDetails, IsActive = pt.IsActive, CreatedAt = pt.CreatedAt
    };

    private static PolicyDto MapPolicyDto(UserPolicy p) => new()
    {
        Id = p.Id, PolicyNumber = p.PolicyNumber,
        PolicyTypeName = p.PolicyType?.Name ?? "",
        Category = p.PolicyType?.Category ?? "",
        Icon = p.PolicyType?.Icon ?? "",
        Status = p.Status, PurchaseDate = p.PurchaseDate, ExpiryDate = p.ExpiryDate,
        Tenure = p.Tenure, PackageAmount = p.PackageAmount,
        DaysLeft = Math.Max(0, (p.ExpiryDate - DateTime.UtcNow).Days),
...
public class ClaimService : IClaimService
{
    ...
    public ClaimService(IClaimRepository repo, IPolicyRepository policyRepo,
        INotificationService notif, IUserRepository userRepo)
    {
        _repo = repo; _policyRepo = policyRepo; _notif = notif; _userRepo = userRepo;
    }

    public async Task<IEnumerable<ClaimDto>> GetMyClaimsAsync(int userId)
    {
        var claims = await _repo.GetByUserIdAsync(userId);
        return claims.Select(MapClaimDto);
    }
    ...
    public async Task<ClaimDto> SubmitClaimAsync(int policyId, SubmitClaimDto dto, int userId, string uploadPath)
    {
        ...
        var claim = new Claim
        {
            UserPolicyId = policyId, ClaimStatus = "Pending",
            ClaimDate = DateTime.UtcNow, MaturityAmount = 0,
    ...
    public async Task<ClaimDto> UpdateClaimAsync(int claimId, UpdateClaimDto dto)
    {
        ...
        return MapClaimDto(claim);
    }

    private static ClaimDto MapClaimDto(Claim c) => new()
    {
        Id = c.Id, ClaimNumber = c.ClaimNumber,
        PolicyNumber = c.UserPolicy?.PolicyNumber ?? "",
        PolicyTypeName = c.UserPolicy?.PolicyType?.Name ?? "",
        ClaimStatus = c.ClaimStatus, ClaimDate = c.ClaimDate,
        MaturityAmount = c.MaturityAmount,
        DocumentsCount = JsonSerializer.Deserialize<List<string>>(c.DocumentsSubmitted)?.Count ?? 0,
        AdminRemarks = c.AdminRemarks,
        UserName = c.UserPolicy?.User?.Name ?? ""
    };
}
...
public class SupportService : ISupportService
{
    ...
    public SupportService(ISupportRepository repo, INotificationService notif)
    {
    ...
    {
        var queries = await _repo.GetByUserIdAsync(userId);
        return queries.Select(MapDto);
    }

    public async Task<IEnumerable<SupportQueryDto>> GetAllQueriesAsync()
    {
        var queries = await _repo.GetAllAsync();
        return queries.Select(MapDto);
    }
    ...
    public async Task<SupportQueryDto> SubmitQueryAsync(CreateSupportQueryDto dto, int userId, string uploadPath)
    {
        ...
        {
            UserId = userId, Subject = dto.Subject, Description = dto.Description,
    ...
    private static SupportQueryDto MapDto(SupportQuery s) => new()
    {
        Id = s.Id, TicketNumber = s.TicketNumber, Subject = s.Subject,
...
public class NotificationService : INotificationService
{
    ...
    private static NotificationDto MapDto(Notification n) => new()
    {
        ...
        ColorCode = n.ColorCode, IsRead = n.IsRead,
        CreatedAt = n.CreatedAt, RelatedFeature = n.RelatedFeature
    };
}
...
public class PaymentService : IPaymentService
{
    ...
    public PaymentService(IPaymentRepository repo, IPolicyRepository policyRepo)
    {
        _repo = repo; _policyRepo = policyRepo;
    }
    ...
    public async Task<PaymentDto> InitiateAsync(InitiatePaymentDto dto, int userId)
    {
        ...
        {
            UserId = userId, UserPolicyId = dto.UserPolicyId,
            Amount = dto.Amount, PaymentDate = DateTime.UtcNow,
            PaymentMethod = dto.PaymentMethod, Status = "Success"
        };
        ...
        {
            Id = created.Id, PaymentNumber = created.PaymentNumber,
            Amount = created.Amount, PaymentDate = created.PaymentDate,
            PaymentMethod = created.PaymentMethod, Status = created.Status,
    ...
    public async Task<IEnumerable<PaymentDto>> GetHistoryAsync(int userId)
    {
        ...
        return payments.Select(p => new PaymentDto
        {
            ...
            Status = p.Status, PolicyNumber = p.UserPolicy?.PolicyNumber ?? ""
        });
    }
}
